...
if ($_SESSION["user"]["typ"] != 1 or empty($_SESSION['db']) or empty($_SESSION['id_db'])){
    ...
    $dotaz_tabulky = "SELECT  tab.table_name as nazev, exp.id_formaty, exp.id_zpusoby, exp.poradi
        ...
        WHERE zad.id_zadani = :db
        AND tab.table_type='BASE TABLE'";
    
    $tabulky = $db->prepare($dotaz_tabulky);
    $tabulky->bindvalue(":db", $_SESSION['id_db']);
    $tabulky->execute();
    ...
    $zpusob->execute();
    $info = $db->prepare("select aktualni_zadani, strhavani FROM nastaveni.zadani where id_zadani = :zad");
    $info -> bindValue(":zad", $_SESSION['id_db']);
    $info->execute();
    
    if(!empty($_POST)){
        try {
            $tabulky2 = $db->prepare($dotaz_tabulky);
            $tabulky2->bindvalue(":db", $_SESSION['id_db']);
            $tabulky2->execute();
            ...
            if($chyba==1){
                ...
                $tabulky3->bindvalue(":db", $_SESSION['id_db']);
                $tabulky3->execute();
                $tabulky3 = $tabulky3->fetchAll();
                foreach ($tabulky2 as $tabulka) {
                    if (empty($tabulka['id_formaty'])){
                        $insert = $db->prepare("insert into nastaveni.export (id_zadani,tabulka,id_formaty,id_zpusoby,poradi) values (:db, :tb, :fo, :zp, :po)");
                        $insert->bindValue(":db", $_SESSION['id_db']);
                        ...
                        $update = $db->prepare("update nastaveni.export set id_formaty=:fo,id_zpusoby=:zp,poradi=:po where id_zadani=:db and tabulka=:tb");
                        $update->bindValue(":db", $_SESSION['id_db']);
                ...
                $existuje = $db -> prepare("select count(*) pocet from nastaveni.zadani where id_zadani = :zad");
                ...
                if (isset($_POST['aktualni_zadani'])){
                    ...
                    $update = $db->prepare("update nastaveni.zadani set aktualni_zadani = 1 where id_zadani = :zad");
                    $update -> bindValue(":zad", $_SESSION['id_db']);
                    $update -> execute();
                }
                if (isset($_POST['strhavani'])){
                    $update = $db->prepare("update nastaveni.zadani set strhavani = :strh where id_zadani = :zad");
